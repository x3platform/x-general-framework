...
public class ApplicationMenuController {
  ...
  private ApplicationService applicationService = AppsContext.getInstance().getApplicationService();
  private AccountService AccountService = MembershipManagement.getInstance().getAccountService();
  private RoleService RoleService = MembershipManagement.getInstance().getRoleService();
  ...
  public String getDynamicTreeView(@RequestBody String data) {
    ...
    if ("menu#applicationId#00000000-0000-0000-0000-000000000001#menuId#00000000-0000-0000-0000-000000000000"
      ...
      if (AppsConfigurationView.getInstance().getHiddenStartMenu()) {
        ...
        outString.append("\"url\":\"" + StringUtil.toSafeJson(url.replace("{treeNodeId}",
    ...
    // ApplicationMenu
    //
    if (keys[0].equals("ApplicationMenu") && keys[4]
      ...
      for (Application item : list) {
        ...
        outString.append("\"url\":\"" + StringUtil.toSafeJson(url.replace("{treeNodeId}", String
          .format("applicationMenu#applicationId#%1$s#menuId#00000000-0000-0000-0000-000000000000",
  ...
  public String getMenus(@RequestBody String data) {
    ...
    /*    if (StringUtil.isNullOrEmpty(parentId) || parentId.equals("0") || parentId.equals("00000000-0000-0000-0000-000000000000")) {
      parentId = application.getId();
    }*/
  ...
  /**
   * 获取菜单 Application , ApplicationMenu
   *
   * @param data 请求的数据内容
   * @return 返回树型结构结果
   */
  ...
  public String getTopMenus(@RequestBody String data) {
    ...
    Application application = applicationService.findOneByApplicationName(applicationName);
    //  获取等级菜单 分为2中情况 ， 一种是 菜单 TopMenu ， 一种是应用 Application
    if ("TopMenu".equals(menuType)) {
      // 当为菜单的时候 , parentId="0"; 判断 读取 应用 还是 Top ， 获取读取 应用
      if ("Application".equals(type)) {
        List<Application> apps = applicationService.findAllByAccountId(accountId);
        outString.append("{\"data\":{");
        outString.append("\"applicationId\":\"" + application.getId() + "\",");
        outString.append("\"applicationName\":\"" + applicationName + "\",");
        outString.append("\"parentId\":\"" + parentId + "\",");
  ...
  private String getMenuScopeChildNodes(String applicationId, String parentId, String menuType,
    ...
    boolean isAdministrator = applicationService
    ...
    if (isAdministrator) {
       ...
       */
      // 可访问成员
      boolean isMember = applicationService.isMember(accountInfo, applicationId);
      // 可访问成员
      if (isMember) {
  ...
  public String getMenuAllBytUser(@RequestBody String data) {
    ...
    if (application != null) {
       ...
       */
      if (applicationService.isMember(accountInfo, application.getId())) {
  ...
  public String getMenuAllByApplicationKey(@RequestBody String data) {
    ...
    Application Application = applicationService.findOneByApplicationKey(applicationKey);
  ...
  public String getUserOrRoleByMenuApplication(@RequestBody String data) {
    ...
    List<Account> accountInfoList = AccountService
      .findAllAccountsByOrganization(organizationUnitId);
    List<Role> roleInfoList = RoleService.findAllRolesByOrganization(organizationUnitId);
    ...
    if (scopes != null && scopes.size() > 0) {
      for (String scopeInfo : scopes) {
        if (scopeInfo.equals("role")) {
          roleInfoList = RoleService.findAllRolesByOrganization(organizationUnitId);
        }
        if (scopeInfo.equals("account")) {
          accountInfoList = AccountService.findAllAccountsByOrganization(organizationUnitId);
        }
      }
      params.put("roleInfoList", roleInfoList);
      params.put("accountInfoList", accountInfoList);
   ...
   *
   * @param data 请求的数据内容
   * @return 返回树型结构结果 主要功能 超级管理员 、  系统管理员  、 管理员 、普通管理员 该功能只能给管理员进行权限赋值 拼装树形结构数据[{id:xxxx,childNodes:[]},{}]
   */
  ...
  private List<Object> getChildApplicationNodes(String parentId, String inApplicationId) {
    ...
    if (!StringUtil.isNullOrEmpty(inApplicationId)) {
      Application info = applicationService.findOne(inApplicationId);
      if (info != null) {
        apps.add(info);
      }
    } else {
      //  查询 带有 登录人 所有的应用
      String accountId = KernelContext.getCurrent().getUser().getId();
      List<Application> Applications = applicationService.findAllByAccountId(accountId);
  ...
  /**
   * 根据角色 绑定菜单 ， 查询来源， 则进行Ever
   */
  @RequestMapping("/bindApplicationMenu")
  public String bindApplicationMenu(@RequestBody String data) {
    try {
      JSONObject req = JSON.parseObject(data);
      String id = req.getString("id"); //
      String dataStr = req.getString("checkedKeys");
      List<String> menus = JSON.parseArray(dataStr, String.class);
      service.bindMenuScopeByRole(id, menus);
    } catch (Exception e) {
