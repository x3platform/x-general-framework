 ...
 * 附件文件信息
 */
public class AttachmentFileInfo implements IAttachmentFileInfo {
  public AttachmentFileInfo() {
    // this.setParent(new AttachmentParentObject());
  }
  ...
  private IAttachmentParentObject parent;

  /**
   * 父级对象
   */
  // @Override
  // public IAttachmentParentObject getParent() {
  //  return parent;
  // }

  // public void setParent(IAttachmentParentObject value) {
  //  parent = value;
  // }
  ...
  public void restore(String id) {
    IAttachmentFileInfo temp = AttachmentStorageContext.getInstance().getAttachmentFileService().findOne(id);

    this.setId(temp.getId());
    this.setEntityId(temp.getEntityId());
    this.setEntityClassName(temp.getEntityClassName());

    // this.getParent().setAttachmentFolder(temp.getVirtualPath().substring(0, temp.getVirtualPath().indexOf("/")).replace("{uploads}", ""));
    // this.getParent().setAttachmentEntityClassName(KernelContext.parseObjectType(AttachmentFileInfo.class));
