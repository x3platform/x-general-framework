 ...
 */
@Lazy
@RestController("com.x3platform.membership.controllers.AccountController")
...
public class AccountController {
  ...
  public String save(@RequestBody String data) throws Exception {
    ...
    MembershipManagement.getInstance().getAccountLogService().log(entity.getId(),
      "基础平台.人员及权限管理.帐号管理." + operationName, description,
  ...
  /**
   * 原先直接调用add,不改变其他影响，直接使用add ，使用请注意
   * 创建账号，微服务，直接调用服务方式
   */
  @RequestMapping("/add")
   ...
   * 根据角色标识查询相关的帐号信息
   */
  @RequestMapping("/findAllByRoleId")
  public String findAllByRoleId(@RequestBody String data) {
    JSONObject req = JSON.parseObject(data);
    
    String roleId = req.getString("roleId");
    
    List<Account> list = service.findAllByRoleId(roleId);
  ...
  public String getOwnAccount(@RequestBody String data) {
    ...
    List<Account> list = service.findAllAccountsByOrganization(organizationUnitId);
    return "{\"data\":" + JSON.toJSONString(list) + ","
      + MessageObject.stringify("0", I18n.getStrings().text("msg_query_success"), true) + "}";
  }
  ...
  public String create(@RequestBody String data) {
    ...
    entity.setLoginName(TEXT_EMPTY);
    entity.setStatus(1);
  ...
  /**
   * @param data {"organizationUnitId":"organizationUnitId"}
   *             根据组织结构，查询组织结构下所有人员
   */
  @RequestMapping("/findAllAccountsByOrganization")
  public String findAllAccountsByOrganization(@RequestBody String data) {
    // 查询当前组织或者当前组织下所有的
    JSONObject req = JSON.parseObject(data);
    String organizationUnitId = req.getString("organizationUnitId");
    List<Account> accountInfo = service.findAllAccountsByOrganization(organizationUnitId);
    return "{\"data\":" + JSON.toJSONString(accountInfo) + "," + MessageObject.stringify("0", I18n.getStrings().text("msg_query_success"), true) + "}";
  }
}
