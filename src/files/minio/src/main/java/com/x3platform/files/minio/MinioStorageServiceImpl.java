...
public final class MinioStorageServiceImpl implements ObjectStorageService {
  @Autowired
  ...
  public int save(ObjectStorageFile entity) {
    ...
    if (StringUtil.isNullOrEmpty(entity.getBucketName())) {
      entity.setBucketName(minioProperties.getDefaultBucketName());
    }
    ...
    try {
      InputStream inputStream = ByteUtil.toStream(entity.getFileData());
      ...
      InternalLogger.getLogger().error("minio putObject Exception", ex);
    }
    ...
    if (account == null) {
      ...
      if (entity != null) {
        ...
        try {
          ...
          InternalLogger.getLogger().error("minio removeObject Exception", ex);
        }
      }
    }
    return 0;
  ...
  public ObjectStorageFile copy(ObjectStorageFile param, String entityClassName, String entityId) {
    ...
    try {
      ...
      InternalLogger.getLogger().error("MinioStorageServiceImpl.copy Exception", ex);
    }
